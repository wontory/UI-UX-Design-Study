...
var checkParams = function (config) {
    ...
    if (!config) { throw new Error("No config object has been provided."); }
    if (typeof config.useTransform !== "boolean") { config.useTransform = true; }
    if (typeof config.gutter !== "number") { config.gutter = DEFAULT_GUTTER; }
...
MagicGrid.prototype.init = function init() {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        if (this.animate) { style.transition = (this.useTransform ? "transform" : "top, left") + " 0.2s ease"; }
    }
    this.started = true;
};
...
MagicGrid.prototype.setup = function setup() {
    ...
    var cols = [];
    if (this.maxColumns && numCols > this.maxColumns) { numCols = this.maxColumns; }
...
MagicGrid.prototype.nextCol = function nextCol(cols, i) {
    if (this.useMin) { return getMin(cols); }
...
MagicGrid.prototype.positionItems = function positionItems() {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        if (this.useTransform) { item.style.transform = "translate(" + left + ", " + top + ")"; }
        else {
            item.style.top = top;
